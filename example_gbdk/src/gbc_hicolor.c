...
void hicolor_start(const hicolor_data * p_hicolor, uint8_t hicolor_bank) NONBANKED {
    ...
    // Make a stack local copy of the struct so that the bank where it resides can be switched out
    hicolor = *p_hicolor;
    // Copy address of palette into local var used by HiColor ISR
    p_hicolor_palettes = hicolor.p_palette;
    hicolor_palettes_bank = hicolor.bank_pal;
    ...
    hicolor_last_scanline = (hicolor.height_in_tiles > DEVICE_SCREEN_HEIGHT) ? (DEVICE_SCREEN_PX_HEIGHT - 1) : ((hicolor.height_in_tiles << 3) - 1);

    // Load the first 256 tiles or less, then the remaining 256 tiles into the alternate bank
    VBK_REG = VBK_BANK_0;
    set_bkg_data(0u, MIN(hicolor.tile_count, 256), hicolor.p_tiles);
    VBK_REG = VBK_BANK_1;
    if (hicolor.tile_count > 256) set_bkg_data(0u, (hicolor.tile_count - 256), hicolor.p_tiles + (256 * 16));


    if (hicolor.bank_map) SWITCH_ROM(hicolor.bank_map);
    // Set the BG Map then attributes
    VBK_REG = VBK_BANK_0;
    set_bkg_tiles(0u, 0u, DEVICE_SCREEN_WIDTH, hicolor.height_in_tiles, hicolor.p_map);
    VBK_REG = VBK_BANK_1;
    set_bkg_tiles(0, 0, DEVICE_SCREEN_WIDTH, hicolor.height_in_tiles, hicolor.p_attribute_map);
